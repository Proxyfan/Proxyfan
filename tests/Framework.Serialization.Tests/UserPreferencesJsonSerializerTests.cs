...
public sealed class UserPreferencesJsonSerializerTests
{
    ...
    /// <summary>
    ///     Verifies that a persisted CaptureMaximumFlows above the supported maximum is replaced
    ///     with the default rather than allowing the in-memory capture cap to grow without bound.
    /// </summary>
    [Test]
    [Arguments(1000001)]
    [Arguments(2000000)]
    ...
    public async Task Deserialize_BoundaryValues_ArePreserved()
    {
        ...
        await Assert.That(preferences.CaptureMaximumFlows).IsEqualTo(1000000);
    }
}
