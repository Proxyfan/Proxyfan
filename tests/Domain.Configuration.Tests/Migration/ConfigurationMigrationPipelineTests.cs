...
public sealed class ConfigurationMigrationPipelineTests
{
    ...
    /// <summary>
    ///     A migrator whose <c>To</c> equals <c>From</c> (no progress) is rejected so the
    ///     pipeline cannot loop forever.
    /// </summary>
    [Test]
    public async Task Migrate_MigratorDoesNotAdvanceVersion_Throws()
    {
        var step = new ConfigurationMigrator
        {
            From = new ConfigurationVersion(1, 0),
    ...
    /// <summary>
    ///     A migrator whose <c>To</c> is beyond the requested target is rejected so the
    ///     pipeline cannot return a schema the caller did not request.
    /// </summary>
    [Test]
    public async Task Migrate_MigratorOvershootsTarget_Throws()
    {
        var step = new ConfigurationMigrator
        {
            From = new ConfigurationVersion(1, 0),
