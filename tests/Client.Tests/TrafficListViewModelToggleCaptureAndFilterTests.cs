...
public sealed class TrafficListViewModelToggleCaptureAndFilterTests
{
    ...
    /// <summary>
    ///     Status-code filtering must be reevaluated when a response arrives.
    /// </summary>
    [Test]
    public async Task VisibleFlows_StatusCodeFilter_ResponseArrival_AddsNowMatchingFlow()
    {
        var bus = new StubBus();
        using var viewModel = new TrafficListViewModel(bus, InlineUserInterfaceScheduler.Instance);
    ...
    /// <summary>
    ///     Status-code filtering must be reevaluated when a response status changes.
    /// </summary>
    [Test]
    public async Task VisibleFlows_StatusCodeFilter_ResponseStatusChange_RemovesNoLongerMatchingFlow()
    {
        var bus = new StubBus();
        using var viewModel = new TrafficListViewModel(bus, InlineUserInterfaceScheduler.Instance);
        var flowId = Guid.NewGuid();
