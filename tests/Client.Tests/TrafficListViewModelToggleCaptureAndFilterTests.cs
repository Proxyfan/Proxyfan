...
public sealed class TrafficListViewModelToggleCaptureAndFilterTests
{
    ...
    /// <summary>
    ///     Status-code filtering must reevaluate hidden flows when a matching response arrives.
    /// </summary>
    [Test]
    public async Task VisibleFlows_StatusCodeFilter_ResponseArrival_AddsNewMatch()
    {
        var bus = new StubBus();
        using var viewModel = new TrafficListViewModel(bus, InlineUserInterfaceScheduler.Instance);
    ...
    /// <summary>
    ///     Status-code filtering must reevaluate visible flows when the response status changes.
    /// </summary>
    [Test]
    public async Task VisibleFlows_StatusCodeFilter_ResponseStatusChange_RemovesStaleMatch()
    {
        var bus = new StubBus();
        using var viewModel = new TrafficListViewModel(bus, InlineUserInterfaceScheduler.Instance);
        var flowId = Guid.NewGuid();
        ...
        bus.PublishResponseReceived(CreateResponseEvent(flowId, 204));
