...
{
    /// <summary>
    ///     Activate triggers RefreshCommand and populates certificate metadata.
    /// </summary>
    [Test]
    public async Task Activate_FirstCall_TriggersRefresh()
    {
        var (viewModel, _, _, store) = Create();

        viewModel.Activate();
        if (viewModel.RefreshCommand.ExecutionTask is { } task)
        {
    ...
    public async Task Activate_CalledTwice_RefreshesOnlyOnce()
    {
        ...
        viewModel.Activate();
        if (viewModel.RefreshCommand.ExecutionTask is { } firstTask)
        {
            await firstTask.ConfigureAwait(false);
        }

        viewModel.Activate();
        if (viewModel.RefreshCommand.ExecutionTask is { } secondTask)
        {
