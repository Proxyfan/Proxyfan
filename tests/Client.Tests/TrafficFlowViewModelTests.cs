...
public sealed class TrafficFlowViewModelTests
{
    ...
    /// <summary>
    ///     A failed status propagates Fail() on the underlying domain flow.
    /// </summary>
    [Test]
    public async Task UpdateStatus_WithFailedEvent_TransitionsViewModelToFailed()
    {
        var requestEvent = CreateRequestEvent();
        var viewModel = new Client.Traffic.ViewModels.TrafficFlowViewModel(requestEvent, 1);
    ...
    /// <summary>
    ///     An aborted status propagates Abort() on the underlying domain flow.
    /// </summary>
    [Test]
    public async Task UpdateStatus_WithAbortedEvent_TransitionsViewModelToAborted()
    {
        var requestEvent = CreateRequestEvent();
        var viewModel = new Client.Traffic.ViewModels.TrafficFlowViewModel(requestEvent, 1);
    ...
    /// <summary>
    ///     A Complete status applied when the view model is already Complete does not throw.
    /// </summary>
    [Test]
    public async Task UpdateStatus_CompleteWhenAlreadyComplete_DoesNotThrow()
    {
        var requestEvent = CreateRequestEvent();
        var viewModel = new Client.Traffic.ViewModels.TrafficFlowViewModel(requestEvent, 1);
