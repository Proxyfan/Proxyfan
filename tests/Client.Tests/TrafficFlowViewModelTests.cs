...
public sealed class TrafficFlowViewModelTests
{
    ...
    /// <summary>
    ///     A failed status updates only the bindable projection.
    /// </summary>
    [Test]
    ...
    /// <summary>
    ///     An aborted status updates only the bindable projection.
    /// </summary>
    [Test]
    ...
    /// <summary>
    ///     A Complete status updates only the bindable projection, even when repeated.
    /// </summary>
    [Test]
    public async Task UpdateStatus_CompleteEvent_DoesNotMutateDomainFlow()
    {
        var requestEvent = CreateRequestEvent();
        var viewModel = new Client.Traffic.ViewModels.TrafficFlowViewModel(requestEvent, 1);
