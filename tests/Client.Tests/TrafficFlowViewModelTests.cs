...
public sealed class TrafficFlowViewModelTests
{
    ...
    /// <summary>
    ///     A failed status sets the view model FlowStatus to Failed.
    /// </summary>
    [Test]
    public async Task UpdateStatus_WithFailedEvent_SetsFlowStatusToFailed()
    {
        var requestEvent = CreateRequestEvent();
        var viewModel = new Client.Traffic.ViewModels.TrafficFlowViewModel(requestEvent, 1);
    ...
    /// <summary>
    ///     An aborted status sets the view model FlowStatus to Aborted.
    /// </summary>
    [Test]
    public async Task UpdateStatus_WithAbortedEvent_SetsFlowStatusToAborted()
    {
        var requestEvent = CreateRequestEvent();
        var viewModel = new Client.Traffic.ViewModels.TrafficFlowViewModel(requestEvent, 1);
    ...
    /// <summary>
    ///     Calling UpdateStatus twice with the same Complete event does not throw.
    /// </summary>
    [Test]
    public async Task UpdateStatus_CalledTwiceWithComplete_DoesNotThrow()
    {
        var requestEvent = CreateRequestEvent();
        var viewModel = new Client.Traffic.ViewModels.TrafficFlowViewModel(requestEvent, 1);
