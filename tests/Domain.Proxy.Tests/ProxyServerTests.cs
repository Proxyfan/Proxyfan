...
/// <summary>Tests for <see cref="ProxyServer" />.</summary>
[NotInParallel]
internal sealed class ProxyServerTests
{
    private StubProxyListener _listener = null!;
    private StubDomainEventBus _eventBus = null!;
    private StubOptionsMonitor<ProxyOptions> _optionsMonitor = null!;
    ...
    /// <summary>Verifies that a newly constructed server has <see cref="ProxyStatus.Stopped" /> status.</summary>
    [Test]
    ...
    /// <summary>Verifies that <see cref="ProxyServer.BoundPort" /> is null before starting.</summary>
    [Test]
    ...
    /// <summary>Verifies that <see cref="ProxyServer.StartAsync" /> transitions status to Running.</summary>
    [Test]
    ...
    /// <summary>Verifies that starting sets <see cref="ProxyServer.BoundPort" />.</summary>
    [Test]
    ...
    /// <summary>Verifies that starting when already running is a no-op and returns success.</summary>
    [Test]
    ...
    /// <summary>Verifies that <see cref="ProxyStarted" /> is published when the proxy starts.</summary>
    [Test]
    ...
    /// <summary>Verifies that <see cref="ProxyStarted.Port" /> matches the bound port.</summary>
    [Test]
    ...
    /// <summary>Verifies that a bind failure transitions to Faulted and returns a failure result.</summary>
    [Test]
    ...
    /// <summary>Verifies that a bind failure publishes <see cref="ProxyErrorOccurred" />.</summary>
    [Test]
    ...
    /// <summary>Verifies that a bind failure error is a <see cref="ProxyBindError" />.</summary>
    [Test]
    ...
    /// <summary>Verifies that a faulted server can recover by calling StartAsync again.</summary>
    [Test]
    ...
    /// <summary>Verifies that <see cref="ProxyServer.StopAsync" /> transitions status to Stopped.</summary>
    [Test]
    public async Task StopAsync_WhenRunning_TransitionsToStopped()
    {
        ...
        var result = await server.StopAsync();

        await Assert.That(result.IsSuccess).IsTrue();
        await Assert.That(server.Status).IsEqualTo(ProxyStatus.Stopped);
    }

    /// <summary>Verifies that stopping when already stopped is a no-op and returns success.</summary>
    [Test]
    ...
    /// <summary>Verifies that <see cref="ProxyStopped" /> is published when the proxy stops.</summary>
    [Test]
    ...
    /// <summary>Verifies that <see cref="ProxyServer.RestartAsync" /> stops and restarts atomically.</summary>
    [Test]
    public async Task RestartAsync_WhenRunning_StopsAndStartsAtomically()
    {
        ...
        var result = await server.RestartAsync();
    ...
    /// <summary>Verifies that restarting when stopped just starts the server.</summary>
    [Test]
    ...
    /// <summary>Verifies that restarting publishes both <see cref="ProxyStopped" /> and <see cref="ProxyStarted" />.</summary>
    [Test]
    ...
    /// <summary>Verifies that disposing a running server stops it.</summary>
    [Test]
    ...
    /// <summary>Verifies that disposing a stopped server completes without error.</summary>
    [Test]
    ...
    /// <summary>Verifies that calling <see cref="ProxyServer.DisposeAsync" /> twice does not throw.</summary>
    [Test]
    ...
    /// <summary>Verifies that the server auto-starts when <see cref="ProxyOptions.AutoStart" /> is true.</summary>
    [Test]
    public async Task Constructor_WhenAutoStartEnabled_StartsAutomatically()
    {
        await using var server = CreateServer(new ProxyOptions { AutoStart = true });
    ...
    /// <summary>Verifies that the server does not auto-start when <see cref="ProxyOptions.AutoStart" /> is false.</summary>
    [Test]
    public async Task Constructor_WhenAutoStartDisabled_RemainsStopped()
    {
        await using var server = CreateServer(new ProxyOptions { AutoStart = false });
    ...
    public async Task OptionsChange_WhenRunning_TriggersRestart()
    {
        ...
        _optionsMonitor.RaiseChange(new ProxyOptions { Port = 9999, AutoStart = false });

        // Allow the background restart Task.Run to complete.
        await Task.Delay(TimeSpan.FromMilliseconds(200));
    ...
    /// <summary>
    ///     Verifies that changing <see cref="ProxyOptions" /> while stopped does not start the server.
    /// </summary>
    [Test]
    public async Task OptionsChange_WhenStopped_TriggersRestart()
    {
        ...
        // When stopped, RestartAsync just starts.
        await Assert.That(server.Status).IsEqualTo(ProxyStatus.Running);
    }
    ...
    public async Task ConcurrentStartStop_DoesNotCorruptState()
    {
        ...
        // After all operations, status must be a valid, stable state.
