...
/// <summary>Tests for <see cref="TcpProxyListener" />.</summary>
[NotInParallel]
internal sealed class TcpProxyListenerTests
{
    ...
    private static TcpProxyListener CreateListener(ProxyOptions options)
    {
        var monitor = new StubOptionsMonitor<ProxyOptions>(options);
        var logger = new StubLogger<TcpProxyListener>();
        return new TcpProxyListener(monitor, logger);
    }

    /// <summary>Verifies that starting the listener sets <see cref="TcpProxyListener.IsListening" /> to <see langword="true" />.</summary>
    [Test]
    ...
    /// <summary>Verifies that starting the listener populates <see cref="TcpProxyListener.BoundPort" />.</summary>
    [Test]
    ...
    /// <summary>Verifies that stopping the listener sets <see cref="TcpProxyListener.IsListening" /> to <see langword="false" />.</summary>
    [Test]
    ...
    /// <summary>Verifies that stopping the listener clears <see cref="TcpProxyListener.BoundPort" />.</summary>
    [Test]
    ...
    /// <summary>Verifies that binding to an already-occupied port throws <see cref="ProxyBindException" />.</summary>
    [Test]
    ...
    /// <summary>Verifies that an accepted connection triggers the callback.</summary>
    [Test]
    ...
    /// <summary>Verifies that calling <see cref="TcpProxyListener.StopAsync" /> when not listening does not throw.</summary>
    [Test]
