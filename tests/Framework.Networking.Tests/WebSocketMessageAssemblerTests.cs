...
public sealed class WebSocketMessageAssemblerTests
{
    ...
    /// <summary>
    ///     Verifies that fragmented control frames (FIN=false) are rejected per RFC 6455 § 5.5.
    /// </summary>
    [Test]
    [Arguments(WebSocketOpcode.Ping)]
    [Arguments(WebSocketOpcode.Pong)]
    [Arguments(WebSocketOpcode.Close)]
    public async Task Accept_FragmentedControlFrame_Throws(WebSocketOpcode opcode)
    {
        var assembler = new WebSocketMessageAssembler();
        var fragmentedControl = new WebSocketFrame(isFinalFragment: false, opcode, Array.Empty<byte>(), 2);

        await Assert.That(() => assembler.Accept(fragmentedControl, WebSocketDirection.Inbound, DateTimeOffset.UtcNow))
            .Throws<InvalidDataException>();
    }
}
