...
/// <summary>Tests for <see cref="ConnectionDispatcher" />.</summary>
[NotInParallel]
internal sealed class ConnectionDispatcherTests
{
    ...
    private static StubConnectionHandler MatchesFirstByteHandler(
        byte expectedByte,
        Func<IProxyConnection, CancellationToken, Task>? onHandle = null)
    {
    ...
    private static StubConnectionHandler MatchesPrefixHandler(
        string asciiPrefix,
        Func<IProxyConnection, CancellationToken, Task>? onHandle = null)
    {
    ...
    /// <summary>Verifies that a handler matching HTTP GET bytes is invoked.</summary>
    [Test]
    ...
    /// <summary>Verifies that a handler matching CONNECT bytes is invoked.</summary>
    [Test]
    ...
    /// <summary>Verifies that a handler matching SOCKS4 first byte (0x04) is invoked.</summary>
    [Test]
    ...
    /// <summary>Verifies that a handler matching SOCKS5 first byte (0x05) is invoked.</summary>
    [Test]
    ...
    /// <summary>Verifies that when no handler accepts the bytes, no handler's HandleAsync is invoked.</summary>
    [Test]
    ...
    /// <summary>Verifies that with an empty handler collection the dispatcher closes gracefully.</summary>
    [Test]
    ...
    /// <summary>Verifies that the first handler in registration order that accepts is invoked.</summary>
    [Test]
    ...
    /// </summary>
    [Test]
    public async Task DispatchAsync_HandlerReceivesAllPeekedBytes()
    {
    ...
    /// </summary>
    [Test]
    public async Task DispatchAsync_PartialBytesThenComplete_WaitsAndDispatches()
    {
        var handler = MatchesFirstByteHandler(0x04);
        var dispatcher = CreateDispatcher([handler]);
        var connection = new StubProxyConnection();

        // Write 2 bytes first, then the remaining 6 after a short delay.
    ...
    public async Task DispatchAsync_TimeoutBeforeEnoughData_ClosesGracefully()
    {
        ...
        // Do not write any data — let the timeout fire.
        await dispatcher.DispatchAsync(connection, CancellationToken.None);
