...
/// <summary>
///     Tests for <see cref="ServerSentEventsStore" />.
/// </summary>
[NotInParallel]
public sealed class ServerSentEventsStoreTests
{
    /// <summary>
    ///     Verifies that <see cref="ServerSentEventsStore.Add" /> increases the stored flow count.
    /// </summary>
    [Test]
    public async Task Add_WhenFlowStored_IncreasesCount()
    {
        var store = new ServerSentEventsStore(3);
        var flow = CreateFlow();
    ...
    {
        var store = new ServerSentEventsStore(2);
    ...
    {
        await Assert.That(() => new ServerSentEventsStore(0)).Throws<ArgumentOutOfRangeException>();
    }

    /// <summary>
    ///     Verifies that <see cref="ServerSentEventsStore.Clear" /> removes all stored flows.
    /// </summary>
    [Test]
    ...
    /// <summary>
    ///     Verifies that <see cref="ServerSentEventsStore.GetAll" /> returns newest items first.
    /// </summary>
    [Test]
    public async Task GetAll_WhenFlowsExist_ReturnsNewestFirst()
    {
        ...
        IReadOnlyList<ServerSentEventsFlow> flows = store.GetAll();
    ...
    /// <summary>
    ///     Verifies that <see cref="ServerSentEventsStore.GetAll" /> returns an empty list when the store is empty.
    /// </summary>
    [Test]
    public async Task GetAll_WhenEmpty_ReturnsEmptyList()
    {
        ...
        var flows = store.GetAll();

        await Assert.That(flows.Count).IsEqualTo(0);
    }

    /// <summary>
    ///     Verifies that <see cref="ServerSentEventsStore.GetById" /> returns the stored flow.
    /// </summary>
    [Test]
    ...
    /// <summary>
    ///     Verifies that <see cref="ServerSentEventsStore.GetById" /> returns null when missing.
    /// </summary>
    [Test]
    ...
    {
        var store = new ServerSentEventsStore(32);
    ...
    private ServerSentEventsFlow CreateFlow()
    {
        var traffic = new TrafficFlow(Guid.NewGuid(), "127.0.0.1:8080", DateTimeOffset.UtcNow);
        var flow = new ServerSentEventsFlow(traffic);
        return flow;
    }

    private void ReadFlows(ServerSentEventsStore store, ManualResetEventSlim startSignal, int iterations)
    {
        startSignal.Wait();
    ...
    private void WriteFlows(ServerSentEventsStore store, ManualResetEventSlim startSignal, int iterations)
    {
        startSignal.Wait();
