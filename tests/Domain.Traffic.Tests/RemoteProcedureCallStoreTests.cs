...
/// <summary>
///     Tests for <see cref="RemoteProcedureCallStore" />.
/// </summary>
[NotInParallel]
public sealed class RemoteProcedureCallStoreTests
{
    /// <summary>
    ///     Verifies that <see cref="RemoteProcedureCallStore.Add" /> increases the stored flow count.
    /// </summary>
    [Test]
    public async Task Add_WhenFlowStored_IncreasesCount()
    {
        var store = new RemoteProcedureCallStore(3);
        var flow = CreateFlow();
    ...
    {
        var store = new RemoteProcedureCallStore(2);
    ...
    {
        await Assert.That(() => new RemoteProcedureCallStore(0)).Throws<ArgumentOutOfRangeException>();
    }

    /// <summary>
    ///     Verifies that <see cref="RemoteProcedureCallStore.Clear" /> removes all stored flows.
    /// </summary>
    [Test]
    ...
    /// <summary>
    ///     Verifies that <see cref="RemoteProcedureCallStore.GetAll" /> returns newest items first.
    /// </summary>
    [Test]
    public async Task GetAll_WhenFlowsExist_ReturnsNewestFirst()
    {
        ...
        IReadOnlyList<RemoteProcedureCallFlow> flows = store.GetAll();
    ...
    /// <summary>
    ///     Verifies that <see cref="RemoteProcedureCallStore.GetAll" /> returns an empty list when empty.
    /// </summary>
    [Test]
    ...
    /// <summary>
    ///     Verifies that <see cref="RemoteProcedureCallStore.GetById" /> returns the stored flow.
    /// </summary>
    [Test]
    ...
    /// <summary>
    ///     Verifies that <see cref="RemoteProcedureCallStore.GetById" /> returns null when missing.
    /// </summary>
    [Test]
    ...
    {
        var store = new RemoteProcedureCallStore(32);
    ...
    private RemoteProcedureCallFlow CreateFlow()
    {
    ...
    private void ReadFlows(RemoteProcedureCallStore store, ManualResetEventSlim startSignal, int iterations)
    {
        startSignal.Wait();
    ...
    private void WriteFlows(RemoteProcedureCallStore store, ManualResetEventSlim startSignal, int iterations)
    {
        startSignal.Wait();
