...
public sealed class WebSocketStoreTests
{
    ...
    public async Task Add_WhenSameIdAddedTwice_ReplacesInPlaceWithoutDuplicating()
    {
        var store = new WebSocketStore(3);
        var id = Guid.NewGuid();
        var traffic = new TrafficFlow(id, "127.0.0.1:8080", DateTimeOffset.UtcNow);
        var originalFlow = new WebSocketFlow(traffic);
        var replacementTraffic = new TrafficFlow(id, "127.0.0.1:9090", DateTimeOffset.UtcNow);
        var replacementFlow = new WebSocketFlow(replacementTraffic);
