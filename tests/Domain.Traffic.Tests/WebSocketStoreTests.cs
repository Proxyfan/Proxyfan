...
public sealed class WebSocketStoreTests
{
    ...
    public async Task Add_WhenSameIdAddedTwice_ReplacesInPlaceWithoutDuplicating()
    {
        var store = new WebSocketStore(3);
        var id = Guid.NewGuid();
        var originalTrafficFlow = new TrafficFlow(id, "127.0.0.1:8080", DateTimeOffset.UtcNow);
        var replacementTrafficFlow = new TrafficFlow(id, "127.0.0.1:9090", DateTimeOffset.UtcNow);
        var originalFlow = new WebSocketFlow(originalTrafficFlow);
        var replacementFlow = new WebSocketFlow(replacementTrafficFlow);
