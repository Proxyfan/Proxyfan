...
public sealed class GraphQueryLanguageRequestClassifierTests
{
    ...
    [Test]
    public async Task GetOperationName_GraphQlFragmentThenOperation_ReturnsOperationName()
    {
        var body = "{\"query\":\"fragment UserFields on User { id } query Viewer { viewer { ...UserFields } }\"}";
        var request = BuildJsonPostRequest("https://example.com/graphql", body);
