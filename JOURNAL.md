...
### 2026-06-03 02:48 — [meta,tooling,feedback] Remove "merging is the human's call" restriction from agent instructions
- **Learned:** The "do not merge unless explicitly delegated" rule lived in four places (`.github/copilot-instructions.md` Git/PR section, `feedback-handler/SKILL.md` "What you do NOT own", `work-item/SKILL.md` "What you do NOT own", `work-item/PROCESS.md` Step 8.5, and `feedback-handler/PROCESS.md` Step 7) — they all reinforced each other, so removing one wasn't enough; the user wanted the entire posture inverted to "merge it yourself once it's green". Kept the "never commit directly to `main`" rule untouched — that one is a separate sensible policy about feature-branch workflow.
- **Unclear:** Whether the merge-restriction language exists in any agent-side prompt or runtime config the repo can't see; if so, future sessions may still cite an external version of the rule even after this PR lands.
- **Harder:** The trigger PR (#385) was so badly behind `main` (115+ files of drift) that the merge it asked for surfaced ~1 real conflict in `SocksTunnelHandler.cs` and a lot of auto-resolved noise — handled the doc change first on its own clean branch off `main` so the policy fix wouldn't be entangled with the conflict resolution.
