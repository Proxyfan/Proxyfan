...
{
    /// <summary>
    ///     Returns the GraphQL operation name for the supplied request, or
    ///     <see langword="null" />.
    /// </summary>
    /// <param name="request">The captured HTTP request data.</param>
    /// <returns>The operation name when one can be determined; otherwise <see langword="null" />.</returns>
    public static string? GetOperationName(HypertextTransferProtocolRequestData? request)
    {
        ...
        var path = request.RequestUri.IsAbsoluteUri ? request.RequestUri.AbsolutePath : request.RequestUri.OriginalString;
        if (!HasGraphQueryLanguageContentType(contentTypeHeader) && !HasMatchingPath(path))
        {
            return null;
        }
    ...
    private static bool HasMatchingPath(string? path)
    {
        ...
        return pathOnly.EndsWith("/graphql", StringComparison.OrdinalIgnoreCase)
            || pathOnly.EndsWith("/api/graphql", StringComparison.OrdinalIgnoreCase)
            || pathOnly.EndsWith("/v1/graphql", StringComparison.OrdinalIgnoreCase)
            || pathOnly.EndsWith("/v2/graphql", StringComparison.OrdinalIgnoreCase);
    }
    ...
    private static bool HasRawGraphQueryLanguageContentType(string? contentType)
    {
        if (string.IsNullOrWhiteSpace(contentType))
        {
            return false;
