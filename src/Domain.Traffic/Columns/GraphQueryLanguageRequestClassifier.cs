...
///     when the request is not GraphQL or the operation name cannot be determined.
/// </summary>
public static class GraphQueryLanguageRequestClassifier
{
    ...
    private static string? ReadOperationName(ReadOnlySpan<char> span, int offset)
    {
        ...
        var length = MeasureIdentifier(span, offset);
        if (length == 0)
        {
            return null;
        }

        return new string(span.Slice(offset, length));
    }
