...
public static class GraphQueryLanguageRequestClassifier
{
    ...
    private static string? ExtractFromJsonBody(string json)
    {
        ...
        try
        {
            ...
            return ExtractOperationNameFromSource(queryElement.GetString());
        }
        catch (JsonException)
        {
            return null;
    ...
    private static string? ExtractFromUrlQuery(string queryString)
    {
        ...
        return string.IsNullOrWhiteSpace(operationName)
            ? ExtractOperationNameFromSource(query)
            : operationName;
    }
    ...
    private static bool HasGraphQueryLanguageIndicator(string? urlPath, string? contentType)
    {
        ...
        return urlPath.EndsWith("/graphql", StringComparison.OrdinalIgnoreCase)
            || urlPath.EndsWith("/api/graphql", StringComparison.OrdinalIgnoreCase)
            || urlPath.EndsWith("/v1/graphql", StringComparison.OrdinalIgnoreCase)
            || urlPath.EndsWith("/v2/graphql", StringComparison.OrdinalIgnoreCase);
    }
