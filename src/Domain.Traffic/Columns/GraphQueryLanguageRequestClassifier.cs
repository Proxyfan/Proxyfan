...
public static class GraphQueryLanguageRequestClassifier
{
    ...
    private static string? ExtractFromUrlQuery(string queryString)
    {
        ...
        var value = queryString.StartsWith('?') ? queryString[1..] : queryString;
        ...
        foreach (var pair in pairs)
        {
            ...
            var pairValue = Uri.UnescapeDataString(pair[(separatorIndex + 1)..]);
    ...
    {
        return HasGraphQueryLanguageContentType(contentType) || HasGraphQueryLanguagePath(path);
    }

    private static bool HasGraphQueryLanguagePath(string? path)
    {
    ...
    {
        return HasBlockStringStart(span, offset)
            ? SkipBlockString(span, offset)
            : SkipRegularString(span, offset);
    }
