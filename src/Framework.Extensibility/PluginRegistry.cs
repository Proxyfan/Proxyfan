...
public sealed class PluginRegistry
{
    ...
    public LoadedPlugin TryInitialize(IProxyfanPlugin plugin, IPluginHost host)
    {
        if (!PluginApiVersionChecker.HasCompatibility(host.ApiVersion, plugin.Metadata.ApiVersion))
        {
            var incompatible = new LoadedPlugin(plugin.Metadata, null, false, $"Required API {plugin.Metadata.ApiVersion} is incompatible with host API {host.ApiVersion}.");
        ...
        try
        {
            ...
            var loaded = new LoadedPlugin(plugin.Metadata, plugin, true, null);
            _plugins.Add(loaded);
            return loaded;
        }
        catch (Exception ex)
        {
            var failed = new LoadedPlugin(plugin.Metadata, null, false, ex.Message);
            _plugins.Add(failed);
            return failed;
        }
    }
