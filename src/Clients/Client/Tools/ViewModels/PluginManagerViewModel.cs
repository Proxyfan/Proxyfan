...
public sealed partial class PluginManagerViewModel : ObservableObject, IDisposable
{
    ...
    /// <param name="userInterfaceScheduler">Scheduler used to marshal directory-change callbacks onto the UI thread.</param>
    ...
    private void ApplyDirectoryChanged()
    {
        if (_isDisposed)
        {
            return;
    ...
    {
        _userInterfaceScheduler.Post(ApplyDirectoryChanged);
    }
