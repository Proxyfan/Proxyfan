...
public static class SourceHostExtractor
{
    ...
    {
        return Extract(domainEvent.Request);
    }

    /// <summary>
    ///     Returns the host header value when present and non-blank,
    ///     falling back to the request URI host otherwise. When no request is
    ///     available, returns the tunnel sentinel used by the traffic list.
    /// </summary>
    /// <param name="request">The request data to extract from.</param>
    /// <returns>The host string used to key the source list grouping.</returns>
