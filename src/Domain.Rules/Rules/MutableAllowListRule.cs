...
/// <summary>
///     A mutable Allow List rule suitable for runtime editing from the user interface.
///     When the rule is enabled and at least one pattern is configured, requests whose
///     URL does NOT match any pattern are blocked (HTTP 403). When disabled, or when no
///     patterns are configured, the rule passes through with no effect.
...
/// </summary>
public sealed class MutableAllowListRule : IRequestPhaseRule
{
    /// <summary>
    ///     Raised whenever the rule's enabled state or pattern collection changes.
    /// </summary>
    public event MutableAllowListChanged? Changed;
    ...
    /// <summary>
    ///     Initializes a new empty <see cref="MutableAllowListRule" />.
    /// </summary>
    /// <param name="priority">
    ...
    public MutableAllowListRule(int priority, bool isEnabled)
    {
        Priority = priority;
    ...
    /// <summary>
    ///     Adds a new pattern to the allow list. Duplicate patterns (same pattern text and kind)
    ///     are ignored.
    /// </summary>
    ...
    /// <summary>
    ///     Removes the supplied pattern from the allow list. The first matching entry
