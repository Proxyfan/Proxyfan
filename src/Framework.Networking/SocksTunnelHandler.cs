...
{
    private const int Socks4MaximumRequestLength = 264;
    private const int Socks5MaximumConnectRequestLength = 262;
    private const int Socks5MaximumGreetingLength = 257;
    ...
    {
        var bytes = await SocksHandshakeReader.ReadIntoArrayAsync(connection.Transport.Input, 9, Socks4MaximumRequestLength, cancellationToken).ConfigureAwait(false);
    ...
    {
        var bytes = await SocksHandshakeReader.ReadIntoArrayAsync(reader, 10, Socks5MaximumConnectRequestLength, cancellationToken).ConfigureAwait(false);
    ...
    {
        var bytes = await SocksHandshakeReader.ReadIntoArrayAsync(reader, 2, Socks5MaximumGreetingLength, cancellationToken).ConfigureAwait(false);
