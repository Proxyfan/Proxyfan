...
public interface IProxyListener
{
    ...
    ///     A <see cref="ConnectionAcceptedHandler" /> invoked for each accepted connection.
    ...
    /// </param>
    /// <returns>
    ///     A <see cref="Task" /> that completes when the listener has bound and the accept loop is running.
    /// </returns>
    /// <exception cref="ProxyBindException">
    ///     Thrown when the configured port is already in use or access is denied.
    /// </exception>
    Task StartAsync(ConnectionAcceptedHandler onConnectionAccepted, CancellationToken cancellationToken);
    ...
    /// </param>
    /// <returns>
    ///     A <see cref="Task" /> that completes when the listener has fully stopped.
    /// </returns>
