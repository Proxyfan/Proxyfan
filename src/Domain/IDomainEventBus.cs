...
///     across bounded contexts. Delivery is synchronous and fire-and-forget:
///     <see cref="Publish{TEvent}" /> calls all registered handlers inline on the calling thread.
///     Handler exceptions are caught and do not prevent subsequent handlers from executing.
/// </summary>
public interface IDomainEventBus
{
    ...
    /// </returns>
    IDisposable Subscribe<TEvent>(DomainEventHandler<TEvent> handler) where TEvent : IDomainEvent;
}