...
///     is empty (feature disabled), the request fails for any reason, or the payload is
///     malformed.
/// </summary>
public sealed class HypertextTransferProtocolPluginUpdateFeed : IPluginUpdateFeed
{
    ...
    public async Task<PluginUpdateManifest?> FetchAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            using var response = await _hypertextTransferProtocolClient.SendAsync(request, cancellationToken).ConfigureAwait(false);
